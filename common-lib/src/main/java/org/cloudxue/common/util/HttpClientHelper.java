 ...
 * @Description HTTP协议帮助处理类
 * @Author xuexiao
 * @Date 2022/10/20 9:30 上午
 * @Version 1.0
 **/
@Slf4j
public class HttpClientHelper {
    ...
    /**
     * 客户端和服务端建立连接的超时时长，单位ms
     * 即三次握手完成的最长时间
     */
    private static final int CONNECT_TIMEOUT = 2000;
    /**
     * 客户端从服务器读取数据的时间长度，单位ms
     * 即数据传输过程中数据包之间间隔的最大时间
     */
    private static final int SOCKET_TIMEOUT = 2000;
    /**
     * 从连接池获取一个连接的请求超时时间，单位ms
     * 即连接池中连接不够用时，阻塞等待的超时时间
     */
...
//        monitorExecutor = Executors.newScheduledThreadPool(1);
        monitorExecutor = ThreadUtil.getSeqOrScheduledExecutorService();
        monitorExecutor.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                //输出连接池的状态,仅供测试使用
                /*
                log.debug(" manager.getRoutes().size():" + manager.getRoutes().size());
                log.debug(" status.getAvailable():" + status.getAvailable());
                log.debug(" status.getPending():" + status.getPending());
                log.debug(" status.getLeased():" + status.getLeased());
                log.debug(" status.getMax():" + status.getMax());
                */
            }
    ...
    private static String simpleRequestData(String url, CloseableHttpClient client, HttpRequest request) throws IOException {
        ...
        try {
            ...
            HttpEntity entity = response.getEntity();
            if (entity != null) {
                log.info("响应长报文长度为： " + entity.getContentLength());
                in = entity.getContent();
                result = IOUtils.toString(in, "utf-8");
            }
        } finally {
